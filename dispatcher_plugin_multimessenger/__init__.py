...
def find_config():
    config_file_resolution_order=[
        os.environ.get('CDCI_MM_PLUGIN_CONF_FILE', '.mm_data_server_conf.yml'),
        os.path.join(conf_dir, 'data_server_conf.yml'),
        "/dispatcher/conf/conf.d/mm_data_server_conf.yml",
    ]
    ...
    raise RuntimeError("no mm config found tried: "+", ".join(config_file_resolution_order))
